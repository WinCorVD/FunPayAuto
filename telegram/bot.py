...
class TGBot:
    def __init__(self, cardinal: Cardinal):
        ...
        self.commands_help = {
            "FunPayCardinal": {
                "menu": "открыть меню",
                "key": "секретный ключ для тестирования авто-выдачи",
                "add_chat": "присылать уведомления в этот чат",
                "remove_chat": "не присылать уведомления в этот чат",
                "block_list": "черный список",
                "ban": "добавить пользователя в черный список",
                "unban": "удалить пользователя из черного списка",
                "restart": "полная перезагрузка"
            }
        }
...
Учти, что пока что я никуда не отправляю уведомления.
Если нужно, что бы я присылал уведомления в этот чат, введи команду /add_chat."""
            self.bot.send_message(message.chat.id, text, reply_markup=keyboards.main_menu())
...
🔑 Отправь мне секретный пароль который ты ввел в моих настройках, что бы начать работу 🙂"""
    ...
    def ignore_unauthorized_users(self, call: types.CallbackQuery):
    ...
    def show_menu(self, message: types.Message):
        self.bot.send_message(message.chat.id, "Меню", reply_markup=keyboards.main_menu())

    def send_commands_text(self, message: types.Message):
        self.bot.send_message(message.chat.id, tg_tools.generate_help_text(self.commands_help))
...
Вам придется логиниться на ваш дедик или подходить к компьютеру (или где еще я там у вас) и запускать меня вручную!""",
    ...
    def add_to_block_list(self, message: types.Message):
        ...
        if len(nickname) < 2:
            self.bot.send_message(message.chat.id, "❌ Укажите имя пользователя, которого хотите внести в ЧС.")
            return
    ...
    def remove_from_block_list(self, message: types.Message):
        nickname = message.text.split(" ", 1)
        if len(nickname) < 2:
            self.bot.send_message(message.chat.id, "❌ Укажите имя пользователя, которого хотите удалить из ЧС.")
            return
    ...
    def send_secret_key(self, message: types.Message):
        self.bot.send_message(message.chat.id, f"Ключ для тестирования авто-выдачи: "
                                               f"<code>{self.cardinal.secret_key}</code>", parse_mode="HTML")
    ...
    def switch_setting(self, call: types.CallbackQuery):
        ...
        self.cardinal.MAIN_CFG[section][option] = "0" if value else "1"
...
<b>Бот:</b>
    ...
    def __init_commands(self):
        ...
        self.msg_handler(self.show_menu, commands=["menu"])
        self.msg_handler(self.send_block_list, commands=["block_list"])
        self.msg_handler(self.add_to_block_list, commands=["ban"])
        self.msg_handler(self.remove_from_block_list, commands=["unban"])
        self.msg_handler(self.restart_cardinal, commands=["restart"])
        self.msg_handler(self.send_secret_key, commands=["key"])

        self.msg_handler(self.send_commands_text, func=lambda msg: msg.text == "📟 Команды 📟")
        self.msg_handler(self.send_cp, func=lambda msg: msg.text == "⚙️ Настройки ⚙️")
        self.msg_handler(self.send_about_text, func=lambda msg: msg.text == "🤖 О боте 🤖")
        self.msg_handler(self.send_logs, func=lambda msg: msg.text == "📋 Логи 📋")
        self.msg_handler(self.send_system_info, func=lambda msg: msg.text == "📈 Система 📈")
        self.msg_handler(self.restart_cardinal, func=lambda msg: msg.text == "🔄 Перезапуск 🔄")
        self.msg_handler(self.ask_power_off, func=lambda msg: msg.text == "🔌 Отключение 🔌")
    ...
    def set_commands(self):
        ...
        for module in self.commands_help:
            for command in self.commands_help[module]:
                commands.append(types.BotCommand(f"/{command}", self.commands_help[module][command]))
