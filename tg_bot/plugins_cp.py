...
def init_plugins_cp(cardinal: Cardinal, *args):
    ...
    def open_plugins_list(c: types.CallbackQuery):
        ...
        bot.edit_message_text(f"Здесь вы можете получить информацию о плагине, а так же настроить его.\n\n"
                              ...
                              parse_mode="HTML", reply_markup=keyboards.plugins_list(cardinal, offset))
...
<i>Обновлено:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
        ...
        bot.edit_message_text(text, c.message.chat.id, c.message.id, parse_mode="HTML", reply_markup=keyboard)
        bot.answer_callback_query(c.id)
...
{commands_text}"""
        ...
        bot.edit_message_text(text, c.message.chat.id, c.message.id, parse_mode="HTML",
    ...
    def act_upload_plugin(c: types.CallbackQuery):
        offset = int(c.data.split(":")[1])
        result = bot.send_message(c.message.chat.id, "Отправьте мне плагин.",
